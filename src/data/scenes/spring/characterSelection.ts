...
const generateSpringCharacterSelectionScenes = (): Record<string, Scene> => {
  ...
  const generateSceneId = (visited: CharacterId[]): string => {
    if (visited.length === 0) return 'spring-character-selection';
    const suffix = visited.map(c => (characterOrder.indexOf(c) + 1).toString()).join('');
    return `spring-character-selection-${suffix}`;
  };
...
const springCharacterSelections = generateSpringCharacterSelectionScenes();
